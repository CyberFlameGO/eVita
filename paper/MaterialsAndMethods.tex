...
For this analysis, all attributes that only use data of the first session were used. That means the 'days between sessions' attributes were discarded. First, we ranked the attributes on their information gain, for this the \code{InfoGainAttributeEval} attribute evaluator plus the \code{Ranker} search method have been used. This produces a list of the most important attributes. After this a classification was performed with the \code{ADTree} classifier (using cross-validation, 10 folds). This results in a performance score, confusion matrix, and a decision tree. A decision tree classifier was applied to give a good impression of the decision process. It can easily be compared to the ranking of the attributes and can easily be checked for correctness. If for example the top node of the tree would be an attribute like visiting the home page, then this might indicate something is wrong.
