...
\title{Predicting adherence for the e-Vita program}
...
\begin{abstract}
The aim of this paper is to investigate the log data of the e-Health platform e-Vita and try to predict the adherence of users with data from their first session. Furthermore, predictors for the number of days between the first three sessions were determined. The results were obtained by transforming the log data into a format with users, their sessions and actions within the sessions. Next, attributes and classes have been identified and saved for use with the Weka tool. The results show that predicting the adherence of users based on the first session is possible with a reasonable accuracy of 74\%. However predicting the number of days between sessions is proven impossible, due to statistically insignificant results.
\end{abstract}

\keywords{Data Science, e-Vita, adherence, prediction, Weka}
